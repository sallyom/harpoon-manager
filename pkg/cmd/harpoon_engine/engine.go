...
// TODO: Add other engines, only "raw" now
func start(config string) {
	repoJson, err := ioutil.ReadFile(config)
	if err != nil {
		log.Fatal(err)
	}
	var repo Repo
	...
	s.Cron(repo.Schedule).Do(processRaw)
	s.StartAsync()
	select {}
}

func processRaw(config string) {
	repoJson, err := ioutil.ReadFile(config)
	if err != nil {
		log.Fatal(err)
	}
